...
export default async (
  ...
  if (opts.tasksConcurrency < 1) {
    const err = new Error('Tasks concurrency should be at least 1')
    err['code'] = 'ERR_PNPM_INVALID_TASKS_CONCURRENCY' // tslint:disable-line:no-string-literal
    throw err
  }
  ...
  const limitInstallation = pLimit(opts.tasksConcurrency)
